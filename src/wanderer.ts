...
const wandererTargets = [
  ...
  new WandererTarget(
    ...
    () => $location`VYKEA`,
    () => (getSaleValue($item`one-day ticket to The Glaciest`) * 0.05) / 50
  ),
  new WandererTarget(
    "Beach Buck",
    () => realmAvailable("sleaze") && get("lovebugsUnlocked"),
    () => $location`The Fun-Guy Mansion`,
    () => (getSaleValue($item`one-day ticket to Spring Break Beach`) * 0.05) / 100
  ),
  new WandererTarget(
    "Coinspiracy",
    () => realmAvailable("spooky") && get("lovebugsUnlocked"),
    () => $location`The Deep Dark Jungle`,
    () => (getSaleValue($item`one-day ticket to Conspiracy Island`) * 0.05) / 100
  ),
...
export function determineDraggableZoneAndEnsureAccess(type: DraggableFight = "wanderer"): Location {
  ...
  const best = sortedTargets.find((prospect) => {
    ...
    printHighlight(`Trying to place a wanderer using ${prospect.target.name}`);
    return (
      location &&
