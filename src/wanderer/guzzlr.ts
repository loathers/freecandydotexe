import { buy, craftType, Location, print, retrieveItem, toInt } from "kolmafia";
import { $item, $skill, get, getSaleValue, Guzzlr, have } from "libram";
import {
        ...
        new WandererTarget("Guzzlr", location, guzzlrValue(Guzzlr.getTier()), () => {
          ...
          if (!have(guzzlrBooze)) {
            ...
            if (
              ...
              (!fancy ||
                (fancy &&
                  toInt(have($skill`Expert Corner-Cutter`)) * (5 - get("_expertCornerCutterUsed")) +
                    toInt(have($skill`Rapid Prototyping`)) * (5 - get("_rapidPrototypingUsed")) >
                    0))
            ) {
